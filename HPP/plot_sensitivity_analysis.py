...
for param_group in param_groups:
    ...
    # Define range based on parameter type
    cost_params = ['pv_capex', 'solar_opex', 'wacc', 'wind_capex', 'wind_opex', 'grid_connection']
    if param_group == 'electricity_price':
        # For electricity_price, keep original range 0.75-1.25
        group_data = group_data[(group_data['level'] >= 0.80) & (group_data['level'] <= 1.20)].copy()
    elif param_group in cost_params:
        # For cost parameters, use 0.8-1.2 range (reversed effect)
        group_data = group_data[(group_data['level'] >= 0.8) & (group_data['level'] <= 1.2)].copy()
    ...
    # For cost parameters, show range from high to low (1.2 to 0.8), for electricity price show low to high
    if param_group in cost_params:
        param_range = f"({param_levels.max():.2f} to {param_levels.min():.2f})"
    else:
...
for pos, (idx, row) in enumerate(tornado_df.iterrows()):
    min_val = row['Min']
    max_val = row['Max']
    
    # Calculate percentage change from baseline
    # Ensure sign matches bar direction: negative for left bars, positive for right bars
    if baseline_npv_capex != 0:
        min_pct = (min_val / abs(baseline_npv_capex)) * 100
        max_pct = (max_val / abs(baseline_npv_capex)) * 100
    else:
        min_pct = 0
        max_pct = 0
    
    
    # Position labels at the end of bars with appropriate offset
    y_position = pos  # Use position in sorted dataframe, not original index
    offset = 0.02 * (tornado_df['Min'].min() - tornado_df['Max'].max())
    
    # Min bar label (red, on the left)
    if min_val < 0:
        ax.text(min_val - offset, y_position, f'{min_pct:.1f}%', 
                va='center', ha='right', fontsize=8, fontweight='bold')
    else:
        ax.text(min_val + offset, y_position, f'{min_pct:.1f}%', 
                va='center', ha='left', fontsize=8, fontweight='bold')
    
    # Max bar label (green, on the right)
    if max_val > 0:
        ax.text(max_val + offset, y_position, f'{max_pct:.1f}%', 
                va='center', ha='left', fontsize=8, fontweight='bold')
    else:
        ax.text(max_val - offset, y_position, f'{max_pct:.1f}%', 
                va='center', ha='right', fontsize=8, fontweight='bold')
