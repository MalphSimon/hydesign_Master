...
def save_risk_return_scatter(summary_df, output_dir):
    ...
    for i, row in summary_df.iterrows():
        plt.text(row["NPV CV"] + 0.01, row["Mean NPV"], row["Site"], fontsize=9, va='center')
...
def save_npv_capex_distribution_boxplot(all_site_data_frames, output_dir):
    ...
    plt.savefig(os.path.join(output_dir, "NPV_CAPEX_Boxplot_Percentage.png"), dpi=300, bbox_inches='tight')
    plt.close()
...
def save_multi_site_comparison(site_list, input_dir, output_dir):
    ...
    for idx, site_key in enumerate(site_list):
        ...
        for i, cfg in enumerate(METRICS):
            ...
            if col:
                data = pd.to_numeric(df[col], errors="coerce").dropna()
                y_plot = data * 100 if cfg["ylabel"] == "%" else data
                axes[i].plot(x, y_plot, marker="o", markersize=4, color=color, alpha=0.7)
    ...
    plt.savefig(os.path.join(output_dir, "Financial_Comparison_All_Sites.png"), dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    if all_site_stats:
        ...
        save_risk_return_scatter(summary_df, args.output_dir)
